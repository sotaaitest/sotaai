    ...
    def train(self,n_epochs = 200, sample_interval = 400,n_critic = 5):
        ...
        for epoch in range(n_epochs):
            for i, (imgs, _) in enumerate(dataloader):
                ...
                # Configure input
                real_imgs = Variable(imgs.type(self.Tensor), requires_grad=True)
                ...
                real_grad_out = Variable(self.Tensor(real_imgs.size(0), 1).fill_(1.0), requires_grad=False)
                real_grad = autograd.grad(
                    real_validity, real_imgs, real_grad_out, create_graph=True, retain_graph=True, only_inputs=True
                )[0]
                real_grad_norm = real_grad.view(real_grad.size(0), -1).pow(2).sum(1) ** (p / 2)

                fake_grad_out = Variable(self.Tensor(fake_imgs.size(0), 1).fill_(1.0), requires_grad=False)
