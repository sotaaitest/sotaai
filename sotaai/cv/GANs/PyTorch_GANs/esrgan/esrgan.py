...
from cv.GANs.PyTorch_GANs.esrgan.models import *
from cv.GANs.PyTorch_GANs.esrgan.datasets import *
...
class ESRGAN():
    def __init__(self, lr = 0.0002,b1 = 0.5, b2 = 0.999, latent_dim = 100,hr_height = 256, hr_width = 256, channels = 1,residual_blocks = 23):
        self.lr = lr
        self.b1 = b1
        self.b2 = b2
        ...
        self.generator = GeneratorRRDB(self.channels, filters=64, num_res_blocks=residual_blocks).to(device)
        self.discriminator = Discriminator(input_shape=(self.channels, *self.hr_shape)).to(device)
        ...
        self.Tensor = torch.Tensor #torch.cuda.FloatTensor if torch.cuda.is_available() else torch.Tensor


# ----------
#  Training
# ----------
    def train(self, start_epoch = 0, n_epochs = 200, sample_interval = 100, warmup_batches = 500, lambda_adv = 5e-3, lambda_pixel = 1e-2, checkpoint_interval = -1, dataset_name = 'img_align_celeba'):

        if start_epoch != 0:
            # Load pretrained models
            self.generator.load_state_dict(torch.load("saved_models/generator_%d.pth" % start_epoch))
            self.discriminator.load_state_dict(torch.load("saved_models/discriminator_%d.pth" % start_epoch))
