...
class ClusterGAN():
    def __init__(self, lr = 0.0001,b1 = 0.5, b2 = 0.999, decay = 2.5*1e-5, latent_dim = 100,img_size = 32,channels = 1):
        self.lr = lr
        self.b1 = b1
        self.b2 = b2
        self.latent_dim = latent_dim
        self.decay = decay
        self.n_c = 10 #Latent space info
        self.x_shape = (channels, img_size, img_size)

        # Wasserstein+GP metric flag
        self.wass_metric = True
        ...
        self.generator = Generator_CNN(self.latent_dim, self.n_c, self.x_shape)
        self.encoder = Encoder_CNN(self.latent_dim, self.n_c)
        ...
        ge_chain = ichain(self.generator.parameters(),
                        self.encoder.parameters())

        self.optimizer_GE = torch.optim.Adam(ge_chain, lr=self.lr, betas=(self.b1, self.b2), weight_decay=self.decay)
        self.optimizer_D = torch.optim.Adam(self.discriminator.parameters(), lr=self.lr, betas=(self.b1, self.b2))

    # ----------
    #  Training
    # ----------
    def train(self, n_epochs=200, batch_size = 64, test_batch_size = 5000, n_skip_iter = 5):
        # Latent space info
            ...
            for i, (imgs, itruth_label) in enumerate(dataloader):
                ...
                if (i % n_skip_iter == 0):
                    ...
                    if self.wass_metric:
                        ...
                        # Vanilla GAN loss
                        valid = Variable(self.Tensor(gen_imgs.size(0), 1).fill_(1.0), requires_grad=False)
                ...
                if self.wass_metric:
                    ...
                    # Vanilla GAN loss
                    fake = Variable(self.Tensor(gen_imgs.size(0), 1).fill_(0.0), requires_grad=False)
