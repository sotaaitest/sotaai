...
import cv.GANs.PyTorch_GANs.cogan.mnistm
...
class COGAN():
    def __init__(self, lr = 0.0002,b1 = 0.5, b2 = 0.999, latent_dim = 100,img_size = 32,channels = 1,n_classes = 10):
        ...
        # Initialize models
        self.coupled_generators = CoupledGenerators(self.latent_dim, self.img_size,self.channels)
        self.coupled_discriminators = CoupledDiscriminators(self.img_size,self.channels)
        ...
        if cuda:
            self.coupled_generators.cuda()
            self.coupled_discriminators.cuda()

        # Initialize weights
        self.coupled_generators.apply(weights_init_normal)
        self.coupled_discriminators.apply(weights_init_normal)

        # Optimizers
        self.optimizer_G = torch.optim.Adam(self.coupled_generators.parameters(), lr=self.lr, betas=(self.b1, self.b2))
        self.optimizer_D = torch.optim.Adam(self.coupled_discriminators.parameters(), lr=self.lr, betas=(self.b1, self.b2))
        ...
        for epoch in range(n_epochs):
            for i, ((imgs1, _), (imgs2, _)) in enumerate(zip(dataloader1, dataloader2)):
                ...
                print(
                    ...
                    % (epoch, n_epochs, i, len(dataloader1), d_loss.item(), g_loss.item())
                )
